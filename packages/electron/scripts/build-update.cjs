...
if (!allowUnsigned && (!publisher || !signingCertificate)) {
  console.error('Signed update required. Set JINGLES_WINDOWS_PUBLISHER and CSC_LINK.');
  console.error('For localhost development builds, explicitly set JINGLES_ALLOW_UNSIGNED_UPDATES=1.');
  process.exit(1);
}
