...
router.use((req: Request, res: Response, next: NextFunction) => {
  ...
  if (
    isLocalPosBackendMode() && req.path === '/local/device-control' && localControlToken &&
...
router.post('/local/device-control', async (req: Request, res: Response) => {
  if (!isLocalPosBackendMode()) return res.status(404).json({ error: 'Not found' });
  try {
    ...
    if (!heartbeat || typeof heartbeat !== 'object') return res.json({ upstreamUrl });
